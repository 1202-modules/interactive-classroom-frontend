...
import type { Workspace, WeekDay } from '../types/workspace';
...
export function useWorkspaceSettings(workspace: Workspace | undefined) {
  ...
  const [autostartSchedule, setAutostartSchedule] = useState<Record<WeekDay, { enabled: boolean; start: string; end: string }>>({
  ...
  const setDay = (day: WeekDay, patch: Partial<{ enabled: boolean; start: string; end: string }>) => {
