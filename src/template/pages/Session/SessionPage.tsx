...
import type { SessionDetail, SessionModule, Participant } from '../../types/sessionPage';
import { mockSessionDetail, mockParticipants } from '../../data/mockSessionDetail';
...
function SortableModuleCard({ module, onActivate, onRemove }: {
  ...
  return (
    <Card
      ...
      style={style}
      view="outlined"
      className={`session-page__module-card ${module.is_active ? 'session-page__module-card_active' : ''}`}
    >
...
function ActiveModuleDropZone({ activeModule }: { activeModule: SessionModule | undefined }) {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`session-page__active-zone ${isOver ? 'session-page__active-zone_over' : ''} ${activeModule ? 'session-page__active-zone_filled' : ''}`}
    >
      {activeModule ? (
        <>
    ...
    <div className="session-page">
        ...
        <div className="session-page__header-main">
          ...
          <div className="session-page__header-info">
            ...
            <div className="session-page__header-meta">
              <Label
                theme={sessionDetail.is_stopped ? 'normal' : 'success'}
                size="m"
              >
      ...
      <div className="session-page__content">
        ...
        >
          <div className="session-page__left-panel">
            ...
            <div className="session-page__modules-list">
              ...
              {sessionDetail.session_modules.filter((m) => !m.is_active).length === 0 && (
          ...
          <DragOverlay>
            {activeId ? (
              <Card view="raised" className="session-page__drag-overlay">
                <Icon
                  data={getModuleIcon(
                    sessionDetail.session_modules.find((m) => m.id === activeId)?.type ||
                      'questions'
                  )}
                  size={18}
                />
                <Text variant="body-2">
                  {sessionDetail.session_modules.find((m) => m.id === activeId)?.name}
                </Text>
              </Card>
            ) : null}
          </DragOverlay>
        ...
        <div className="session-page__center-panel">
          <TabProvider value={previewTab} onUpdate={(v) => setPreviewTab(v as PreviewTab)}>
